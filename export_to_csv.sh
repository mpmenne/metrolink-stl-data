...
do
        ...
        no_carriage_returns=${no_ampersands//^M/}
        url_friendly=${no_carriage_returns// /_}
        echo "$url_friendly"
        ...
        WEEKDAY_QUERY="select * from metrolink_stops where stop_name=$col5 and service_id='1_merged_2438691' and $END_OF_LINE_EXCLUSIONS order by arrival_time;"
        echo $WEEKDAY_QUERY
        sqlite3 -csv  metrolink.db <<< $WEEKDAY_QUERY | tee "$WEEKDAY_FILE_NAME" 
        sqlite3 -csv  metrolink.db <<< "select * from metrolink_stops where stop_name=$col5 and service_id='2_merged_2438693' and $END_OF_LINE_EXCLUSIONS order by arrival_time;" | tee "$WEEKEND_FILE_NAME" 
